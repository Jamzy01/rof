...
use rof_rs::object_format::property::Property;

use rof_rs::object_format::rof::Rof;

use rof_rs::object_format::DataValue;

use rof_rs::object_format::data_value::{
...
fn rof_compat_derive_test() {
    let mut user = User::load_from_file("J:\\Programming\\Rust\\rof\\example_objects\\user.rof");
...
fn rof_compat_custom_impl_test() {
    let mut color = Color::load_from_file("J:\\Programming\\Rust\\rof\\example_objects\\color.rof");
...
fn low_level_rof_test() {
    ...
    let computer_structure = computer_rof.get_object().as_struct_structure();
...
fn main() {
    rof_compat_derive_test();
    rof_compat_custom_impl_test();
    rof_compat_custom_impl_test();
    low_level_rof_test();
}
